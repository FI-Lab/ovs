...
static bool atctl_equal(const struct flow *a,const struct flow *b,int domain_counter,enum set_domain_match domain_set);
static struct ofputil_atctl_rule* atctl_find(struct miniflow *mf);
static void atctl_execute(struct ofputil_atctl_rule *rule,struct dp_packet **packets,struct dp_netdev_pmd_thread *pmd);
...
atctl_equal(const struct flow *a,const struct flow *b,int domain_counter,enum set_domain_match domain_match){
	...
	if(domain_match & NW_SRC)
	    memcmp(a->nw_src,b->nw_src,sizeof(a->nw_src));
	else {
	    not_equal = 1;
	    break;
	}
	if(domain_match & NW_DST)
	    memcmp(a->nw_dst,b->nw_dst,sizeof(a->nw_dst));
	else {
	    not_equal = 1;
	    break;
	}
	...
	if(domain_match & TP_SRC)
	    memcmp(a->tp_src,b->tp_src,sizeof(a->tp_src));
	else {
	    not_equal = 1;
	    break;
	}
	if(domain_match & TP_DST)
	    memcmp(a->tp_dst,b->tp_dst,sizeof(a->tp_dst));
	else {
	    not_equal = 1;
	    break;
	}
	if(domain_match & NW_PROTO)
	    memcmp(a->nw_proto,b->nw_proto,sizeof(a->nw_proto));
	else {
	    not_equal = 1;
	    break;
	}
...
static void
atctl_execute(struct ofputil_atctl_rule *rule,struct dp_packet **packets,
	                   struct dp_netdev_pmd_thread *pmd){
    struct dp_netdev_actions *netdev_actions;
    netdev_actions = dp_netdev_actions_create(rule->ofpacts,rule->ofpacts_len);
    dp_netdev_execute_actions(pmd,packets,1,true,
	    netdev_actions->actions,netdev_actions->size);
}
